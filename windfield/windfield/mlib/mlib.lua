	...
	for index1 = #tab, 1, -1 do
		...
		for index2 = #tab, 1, -1 do
			...
			if index1 ~= index2 then
				if
					type(first[1]) == "number" and type(second[1]) == "number" and type(first[2]) == "number" and
						type(second[2]) == "number"
				 then
	...
	for index1 = #tab, 1, -1 do
		...
		for index2 = #tab, 1, -1 do
			...
			if index1 ~= index2 then
				...
				if
					type(first[2]) == "number" and type(second[2]) == "number" and type(first[3]) == "number" and
						type(second[3]) == "number"
				 then
...
local function rotatePoint(x, y, rotation, ox, oy)
	...
	return (x - ox) * math.cos(rotation) + ox - (y - oy) * math.sin(rotation), (x - ox) * math.sin(rotation) +
		(y - oy) * math.cos(rotation) +
		oy
end
...
local function getPolygonSegmentIntersection(x1, y1, x2, y2, ...)
	...
	for i = 1, #input, 2 do
		local x1, y1, x2, y2 =
			getSegmentSegmentIntersection(input[i], input[i + 1], cycle(input, i + 2), cycle(input, i + 3), x1, y1, x2, y2)
		if x1 and not x2 then
			choices[#choices + 1] = {x1, y1}
		elseif x2 then
			choices[#choices + 1] = {x1, y1, x2, y2}
...
local function isPolygonInsidePolygon(polygon1, polygon2)
	...
	for i = 1, #polygon2, 2 do
		local result = false
		result =
			isSegmentInsidePolygon(polygon2[i], polygon2[i + 1], cycle(polygon2, i + 2), cycle(polygon2, i + 3), polygon1)
		if result then
			bool = true
			break
		end
...
local function isSegmentCompletelyInsidePolygon(x1, y1, x2, y2, ...)
	...
	if
		not checkPolygonPoint(x1, y1, polygon) or not checkPolygonPoint(x2, y2, polygon) or
