...
Lynput.s_gamepadButtons = {
	...
	["guide"] = "G_GUIDE",
	["start"] = "G_START",
	["leftstick"] = "G_LEFTSTICK",
	["rightstick"] = "G_RIGHTSTICK",
	["leftshoulder"] = "G_LB",
	["rightshoulder"] = "G_RB",
	["dpup"] = "G_DPAD_UP",
	["dpdown"] = "G_DPAD_DOWN",
	["dpleft"] = "G_DPAD_LEFT",
	["dpright"] = "G_DPAD_RIGHT"
}

Lynput.s_gamepadAxes = {
	["leftx"] = "G_LEFTSTICK_X",
	["lefty"] = "G_LEFTSTICK_Y",
	["rightx"] = "G_RIGHTSTICK_X",
	["righty"] = "G_RIGHTSTICK_Y",
	["triggerleft"] = "G_LT",
	["triggerright"] = "G_RT"
}
...
local function _isCommandValid(command)
	...
	-- Process state
	stateValid = state == "release" or state == "press" or state == "hold"
	...
	if not stateValid then
		...
		stateValid = min < max and min >= -100 and max <= 100
...
function Lynput:getAxis(axis)
	...
	error("Axis->" .. axis .. " is not a valid name. Use LÖVE or Lynput names for axes.")
end
...
function Lynput:bind(action, commands)
	...
	assert(
		type(action) == "string",
		"bad argument #1 to 'Lynput:bind' (string expected, got " ..
	...
	for _, command in ipairs(commands) do
		...
		assert(
			...
			"Could not bind command->" .. command .. -- TODO: Check documentation for valid actions message
...
function Lynput:unbind(action, commands)
	...
	assert(
		type(action) == "string",
		"bad argument #1 to 'Lynput:unbind' (string expected, got " ..
			type(action) .. ")" .. "\nCheck the stack traceback to know where the invalid arguments have been passed"
	)
	...
	for _, command in ipairs(commands) do
		...
		assert(
			self[action] ~= nil,
			"Could not unbind command->" .. command .. "  to action->" .. action .. ", the action is not set."
		)
		-- Is command set?
		local state, input = string.match(command, "(.+)%s(.+)")
		assert(
			self.inputsSet[input][state],
			"Could not unbind command->" .. command .. "  to action->" .. action .. ", the command is not set."
		)
...
function Lynput:unbindAll(action)
	...
	assert(
		type(action) == "string",
		"bad argument #1 to 'Lynput:unbindAll' (string expected, got " ..
			type(action) .. ")" .. "\nCheck the stack traceback to know where the invalid arguments have been passed"
	)

	-- Is action set?
	assert(self[action] ~= nil, "Could not unbind all commands in action->" .. action .. ", the action is not set.")
...
function Lynput:removeAction(action)
	...
	assert(
		type(action) == "string",
		"bad argument #1 to 'Lynput:removeAction' (string expected, got " ..
			type(action) .. ")" .. "\nCheck the stack traceback to know where the invalid arguments have been passed"
	)

	-- Is action set?
	assert(self[action] ~= nil, "Could not remove action->" .. action .. ", this action does not exist.")
